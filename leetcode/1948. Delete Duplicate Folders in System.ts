class Folder {
  ...
  constructor(name: string) {
    this.name = name;
    this.childrenList = [];
    this.childrenMap = new Map<string, Folder>();
    this.isMarked = false;
  }
}
...
class Tree {
  ...
  insertPath(root: Folder, path: string[]) {
    if (path.length === 0) {
      return;
    }
  ...
  generateFolderKey(root: Folder): string {
  ...
  deduplicatedFolders(paths: string[][]): string[][] {
